...
export default function StockMarketChart() {
  ...
  const [zoomedSelection, setZoomedSelection] = React.useState(null);
  ...
  const handleSelection = (chartContext, { xaxis }) => {
    const { min, max } = xaxis[0];
    setZoomedSelection({ min, max });
  };

  const handleResetZoom = () => {
    setZoomedSelection(null);
  };

  return (
    <div>
      <div id="chart">
        <ReactApexChart options={options} series={series} type="candlestick" height={600} onSelection={handleSelection} />
      </div>
