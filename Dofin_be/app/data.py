...
async def get_intraday_quotes(symbol: str):
...
async def get_stock_exchange(symbol: str):
    headers = {"JWTToken": jwt_token}
    async with aiohttp.ClientSession() as session:
        async with session.get(
...
async def get_all_stock_exchange():
    headers = {"JWTToken": jwt_token}
    async with aiohttp.ClientSession() as session:
        async with session.get(
            ...
            respone_data = await response.json()
            mapping_exchange(respone_data)


def mapping_financial(raw_data):
    symbol_info_list = []
    for data in raw_data:
        date = datetime_to_date(data.get("Date", None))
        if date is None:
            continue
    ...
    with ScopedSession() as session:
        save_finance(Tickers, Finances, session, symbol_info_list)


def save_finance(parrent_model, model, session, values_to_insert):
...
async def allLastest_financialinfo():
    headers = {"JWTToken": jwt_token}
    async with aiohttp.ClientSession() as session:
        async with session.get(
            ...
            respone_data = await response.json()
            mapping_financial(respone_data)
...
def mapping_exchange(raw_data):
    ...
    with ScopedSession() as session:
        save_2_db(Tickers, session, symbol_info_list)
...
async def history_price(symbol):
    headers = {"JWTToken": jwt_token}
    async with aiohttp.ClientSession() as session:
        async with session.get(
            urls
...
async def crawl_data():
    ...
    for symbol in symbols:
        await history_price(symbol)


def mapping_history(raw_data):
    price_data_list = []
    for data in raw_data:
        date = datetime_to_date(data.get("Date"))
    ...
    with ScopedSession() as session:
        save_2_D1(D1, session, price_data_list)


def save_2_D1(model, session, values_to_insert):
    for item in values_to_insert:
        stmt = insert(model).values(**item)
        stmt = stmt.on_conflict_do_update(
            index_elements=["symbol", "date"],
...
async def current_price_all_market():
    headers = {"JWTToken": jwt_token}
    async with aiohttp.ClientSession() as session:
        async with session.get(
...
def update_db(model, session, values_to_update):
...
def json_to_list_of_dicts(file_path):
    try:
        # Open the JSON file
...
async def import_manual_data():
    symbol_info_list = json_to_list_of_dicts("manual_data.json")
    with ScopedSession() as session:
        update_db(Tickers, session, symbol_info_list)


if __name__ == "__main__":
    asyncio.run(authenticate())
    asyncio.run(allLastest_financialinfo())
